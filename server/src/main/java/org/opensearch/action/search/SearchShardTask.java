...
public class SearchShardTask extends WorkloadGroupTask implements SearchBackpressureTask {
     ...
     * Used by the DataFusion engine to fire its CancellationToken as soon as the
     * OpenSearch task is cancelled (HTTP disconnect, _tasks/_cancel, timeout),
     * rather than waiting until the next record-batch boundary.
     */
    ...
    public void setCancellationListener(Runnable listener) {
        ...
        // If the task was already cancelled before we registered, fire immediately.
